var gui = (function() {
  ...
  var current = "close";
  var ui;
  var content, mainMenu, menu, timebarre, topText, notes;
  var gender, genderIM;
  var isOpen = false;
  ...
  gui = {
    ...
    init: function(container) {
      ...
      content.style.cssText =
        "position:absolute; top:0; left:0; pointer-events:none; width:100%; height:100%; overflow:hidden; ";
      container.appendChild(content);

      gender = document.createElement("div");
      gender.style.cssText =
        "position: absolute; bottom:50px; left:10px; pointer-events:auto; width:60px; height:90px; cursor:pointer;";

      topText = document.createElement("div");
      topText.style.cssText =
        "position: absolute; top:0px; right:0px; color:#CCC; font-size: 14px; margin:0px 0px; padding: 0px 15px; line-height:40px; pointer-events:none; width:60px; height:40px; text-align: center; ";
      ...
      mainMenu.style.cssText =
        "position:absolute; top:40px; right:0; pointer-events:none; width:200px;";

      menu = document.createElement("div");
      menu.style.cssText =
        "position: absolute; top:0px; left:0px; height:40px; width:100%; pointer-events:none; ";
    ...
    addButton: function(i) {
      ...
      b.style.cssText =
        "color:#CCC;  font-size: 14px;  margin:0px 0px; padding: 0px 15px; line-height:40px; position:relative; pointer-events:auto; height:40px; display:inline-block; text-align:center; cursor:pointer; transition:all 0.3s ease;";
      b.textContent = BB[i];
      b.id = i;
      ...
      b.addEventListener(
        ...
        function(e) {
          ...
          this.style.background = "rgba(153,153,153,0.5)";
        },
        false
      );
      b.addEventListener(
    ...
    view: function() {
      ...
      ui.add("Bool", {
        name: "MID RESOLUTION",
        value: view.pixelRatio === 1 ? false : true,
        p: 60,
        bColor: hc,
        inh: hb
      }).onChange(view.setPixelRatio);
      ...
      ui.add("Bool", {
        ...
        bColor: hc,
        inh: hb
      }).onChange(view.addShadow);
      ui.add("Bool", {
        name: "SKELETON",
        value: main.model.isSkeleton,
        p: 60,
        bColor: hc,
        inh: hb
      }).onChange(function(b) {
      ...
      ui.add("Bool", {
        ...
        bColor: hc,
        inh: hb
      }).onChange(view.showSky);

      ui.add("title", { name: " ", h: 30 });

      ui.add(params, "gammaInput", {
        type: "Bool",
        fontColor: "#EEE",
        p: 60,
        bColor: hc,
      ...
      ui.add("list", {
        name: "type",
        bColor: "#b2b2b2",
        list: ["None", "Linear", "Reinhard", "Uncharted2", "Cineon"],
        fontColor: "#333",
        value: params.tone,
        h: 30
      }).onChange(function(v) {
    ...
    video: function() {
      ...
      ui.add("number", {
        name: "resolution",
        value: view.videoSize,
        precision: 0,
        step: 10
      }).onChange(view.setVideoSize);
      ui.add("button", { name: "START", h: 20, p: 0 }).onChange(function() {
        view.startCapture();
      });
      ui.add("button", { name: "STOP", h: 20, p: 0 }).onChange(function() {
        view.saveCapture();
      });
    },
    ...
    material: function() {
      ...
      ui.add(settings, "color", {
        type: "color",
        ctype: "hex",
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "muscles", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "oamap", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "lightmap", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "metalness", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "roughness", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "skinAlpha", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "shininess", {
        min: 0,
        max: 200,
        fontColor: "#D4B87B",
        precision: 0
      }).onChange(gui.applyMaterial);
      ui.add(settings, "opacity", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
      ui.add(settings, "reflectivity", {
        min: 0,
        max: 1,
        fontColor: "#D4B87B"
      }).onChange(gui.applyMaterial);
    ...
    animation: function() {
      ...
      ui.add("slide", {
        ...
        step: 1,
        stype: 1
      }).onChange(view.setFramerate);

      ui.add("slide", {
        name: "animation",
        min: -1,
        max: 1,
        value: main.timescale,
        precision: 2,
        stype: 1
      }).onChange(main.setTimescale);
      ui.add("Bool", {
        name: "LOCK HIP",
        value: main.model.isLockHip,
        p: 60,
        bColor: hc,
        inh: hb
      }).onChange(main.lockHip);
      ui.add("button", {
        name: "LOAD BVH",
        fontColor: "#D4B87B",
        h: 40,
        drag: true,
        p: 0
      }).onChange(main.loadAnimation);

      var an = main.animations,
        name;

      for (var i = 0; i < an.length; i++) {
        name = an[i];
        //ui.add( 'button', { name:name, p:0 }).onChange( avatar.play );
        ui.add("button", { name: name, p: 0 }).onChange(function(n) {
          main.model.play(n);
        });
      }
...
var Timebarre = function(p, sel) {
  ...
  this.playIcon =
    "<svg width='18px' height='17px'><path fill='#CCC' d='M 14 8 L 5 3 4 4 4 13 5 14 14 9 14 8 Z'/></svg>";
  this.pauseIcon =
    "<svg width='18px' height='17px'><path fill='#CCC' d='M 14 4 L 13 3 11 3 10 4 10 13 11 14 13 14 14 13 14 4 M 8 4 L 7 3 5 3 4 4 4 13 5 14 7 14 8 13 8 4 Z'/></svg>";
  ...
  this.content.style.cssText =
    "position:absolute; bottom:0; left:0px; width:100%; height:50px; pointer-events:none; display:none; ";
  this.parent.appendChild(this.content);

  this.timeInfo = document.createElement("div");
  this.timeInfo.style.cssText =
    "position:absolute; bottom:36px; left:60px; width:200px; height:10px; pointer-events:none; color:#CCC; ";
  this.content.appendChild(this.timeInfo);

  this.timeline = document.createElement("div");
  this.timeline.style.cssText =
    "position:absolute; bottom:20px; left:60px; width:" +
    this.width +
    "px; height:5px; border:3px solid rgba(255,255,255,0.2); pointer-events:auto; cursor:pointer;";
  this.content.appendChild(this.timeline);

  this.framer = document.createElement("div");
  this.framer.style.cssText =
    "position:absolute; top:0px; left:0px; width:1px; height:5px; background:#CCC; pointer-events:none;";
  this.timeline.appendChild(this.framer);

  this.playButton = document.createElement("div");
  this.playButton.style.cssText =
    "position:absolute; top:5px; left:10px; width:18px; height:18px; pointer-events:auto; cursor:pointer; border:3px solid rgba(255,255,255,0.2); padding: 5px 5px;";
...
Timebarre.prototype = {
  ...
  play_over: function(e) {
    ...
    this.playButton.childNodes[0].childNodes[0].setAttribute(
      "fill",
      this.select
    );
  },
